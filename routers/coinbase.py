...
async def _validate_view_only_credentials(
    ...
    if any(permissions[name] for name in ("can_trade", "can_transfer", "can_receive")):
        raise HTTPException(
            status_code=400,
            detail=(
...
def _write_spot_position_rows(
    ...
    for position in positions:
        ...
        if (
            not include_zero
            and _decimal_or_zero(position.get("total_balance_crypto")) == 0
        ):
            continue
...
async def get_coinbase_balance(
    ...
    intx_capsule: str | None = Query(
        None,
        max_length=COINBASE_CAPSULE_MAX_LENGTH,
        description=(
            "Optional separate encrypted Coinbase Perpetuals/INTX access key. "
            "When omitted, the primary key is reused for backward compatibility."
        ),
    ),
    ...
    intx_credentials = (
        decrypt_coinbase_credentials(intx_capsule) if intx_capsule else credentials
    )
        ...
        if include_portfolios:
            ...
            portfolio_breakdowns.extend(
                await _fetch_coinbase_portfolio_breakdowns(
                    client,
                    intx_credentials.key_name,
                    intx_credentials.key_secret,
                )
            )
