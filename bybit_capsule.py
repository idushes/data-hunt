...
BYBIT_CAPSULE_PREFIX = "dhb1"
BYBIT_CAPSULE_MAX_LENGTH = 4096
BYBIT_API_KEY_MAX_LENGTH = 256
BYBIT_API_SECRET_MAX_LENGTH = 256
...
def _load_keyring() -> tuple[str, dict[str, bytes]]:
    ...
    if not active_key_id or not raw_keyring:
        raise HTTPException(
            status_code=503,
            detail="Bybit credential capsules are not configured",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail="Bybit credential capsule configuration is invalid",
        ) from exc


def _associated_data(key_id: str) -> bytes:
    return f"data-hunt:bybit:capsule:v1:{key_id}".encode()


def encrypt_bybit_credentials(api_key: str, api_secret: str) -> str:
    if not api_key or len(api_key) > BYBIT_API_KEY_MAX_LENGTH:
        raise HTTPException(status_code=400, detail="Invalid Bybit API key")
    if not api_secret or len(api_secret) > BYBIT_API_SECRET_MAX_LENGTH:
        raise HTTPException(status_code=400, detail="Invalid Bybit API secret")
    ...
    return ".".join(
        [BYBIT_CAPSULE_PREFIX, active_key_id, _encode_base64url(nonce + ciphertext)]
    )


def decrypt_bybit_credentials(capsule: str) -> BybitCredentials:
    if not capsule or len(capsule) > BYBIT_CAPSULE_MAX_LENGTH:
        raise HTTPException(status_code=400, detail="Invalid Bybit access key")

    try:
        prefix, key_id, encoded_payload = capsule.split(".", 2)
        if prefix != BYBIT_CAPSULE_PREFIX:
        ...
        raise HTTPException(status_code=400, detail="Invalid Bybit access key") from exc
